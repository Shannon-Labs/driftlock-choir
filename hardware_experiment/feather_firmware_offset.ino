/*
 * Adafruit Feather M4 Express - Offset Signal Generator
 * Part of E1 Chronometric Interferometry Hardware Experiment
 * 
 * This firmware generates a precise 1.000100 MHz offset signal
 ...
 * Frequency: 1.000100 MHz (100 Hz offset from reference)
 * Communication: USB Serial for timing coordination
 * 
 * The 433rd harmonic (433.1 MHz) will be captured by RTL-SDR
 */
...
const float BASE_FREQ_HZ = 1000100.0;  // 1.000100 MHz (100 Hz offset)
...
void setup() {
  ...
  Serial.println("=== Feather M4 Offset Signal Generator ===");
  Serial.println("E1 Chronometric Interferometry Hardware Experiment");
  Serial.println("Offset Frequency: 1.000100 MHz (+100 Hz offset)");
...
void loop() {
  ...
  // Blink LED to show activity (different pattern than reference)
  if (signal_active) {
    digitalWrite(LED_PIN, (millis() / 150) % 2); // Faster blink for offset generator
  } else {
    digitalWrite(LED_PIN, (millis() / 1500) % 2);
  }
...
void start_signal_generation() {
  Serial.println("Starting offset signal generation...");
  ...
  Serial.println("Offset signal generation started!");
}

void stop_signal_generation() {
  Serial.println("Stopping offset signal generation...");
  ...
  Serial.println("Offset signal generation stopped.");
...
void print_status() {
  if (signal_active) {
    Serial.print("OFFSET ACTIVE - Runtime: ");
...
void print_detailed_status() {
  Serial.println("=== Offset Signal Generator Status ===");
  Serial.print("Signal State: ");
  Serial.println(signal_active ? "ACTIVE" : "STOPPED");
  Serial.print("Offset Frequency: ");
  Serial.print(BASE_FREQ_HZ / 1000000.0, 6);
  Serial.println(" MHz");
  Serial.print("Frequency Offset: +");
  Serial.print((BASE_FREQ_HZ - 1000000.0), 1);
  Serial.println(" Hz");
  Serial.print("Target Harmonic: 433.1 MHz (");
  Serial.print(433100000.0 / BASE_FREQ_HZ, 0);
...
void print_help() {
  ...
  Serial.println("START    - Start offset signal generation");
  Serial.println("STOP     - Stop offset signal generation");
