...
class ChannelSimulator:
    ...
    def apply_channel(
        self, signal: np.ndarray, channel_model: ChannelModel, tx_frequency: Hertz
    ) -> np.ndarray:
        """
        ...
        if channel_model.doppler_shift != 0:
            output_signal = self._apply_doppler_shift(
                output_signal, channel_model.doppler_shift
            )
        ...
        if len(channel_model.path_delays) > 0:
            ...
            first_path_distance = PhysicalConstants.ps_to_meters(
                channel_model.path_delays[0]
            )
            path_loss_db = self._calculate_free_space_path_loss(
                first_path_distance, tx_frequency
            )
    ...
    def create_multipath_channel(
        self, delays: List[Picoseconds], powers: List[float], doppler_shift: Hertz = 0.0
    ) -> ChannelModel:
        """
        ...
        if len(delays) > 1:
            ...
            delay_spread = Picoseconds(
                np.sqrt(np.mean([(d - delay_mean) ** 2 for d in delays]))
            )
        else:
            delay_spread = Picoseconds(0.0)
    ...
    def create_indoor_channel(
        self, distance_m: float, tx_frequency: Hertz
    ) -> ChannelModel:
        """
    ...
    def _apply_doppler_shift(
        self, signal: np.ndarray, doppler_shift: Hertz
    ) -> np.ndarray:
        """
    ...
    def _calculate_free_space_path_loss(
        self, distance: Meters, frequency: Hertz
    ) -> float:
        """
    ...
    def add_thermal_noise(
        ...
        if snr_db is not None:
            ...
            noise_power_dbm = (
                PhysicalConstants.THERMAL_NOISE_DBM_PER_HZ
    ...
    def estimate_channel_from_beat_note(self, beat_note: BeatNoteData) -> ChannelModel:
        ...
        if len(significant_indices) > 0:
            ...
            delays = [
                PhysicalConstants.seconds_to_ps(idx / self.sampling_rate)
    ...
    def get_channel_frequency_response(
        ...
        frequency_vector = np.fft.fftfreq(
            len(impulse_response), 1.0 / self.sampling_rate
        )

        # Return only positive frequencies
        positive_freq_idx = frequency_vector >= 0
        return (
            frequency_vector[positive_freq_idx],
            frequency_response[positive_freq_idx],
        )

    def create_rf_multipath_channel(
        self,
        delay: Picoseconds,
        multipath_taps: List[float],
        multipath_delays: List[float],
    ) -> ChannelModel:
        """
        Create a realistic RF multipath channel model for chronometric interferometry.

        Args:
            delay: Primary path delay in picoseconds
            multipath_taps: Multipath tap amplitudes (relative to primary path)
            multipath_delays: Additional delays for multipath components in picoseconds

        Returns:
            RF multipath channel model
        """
        # Create delays: primary path + multipath components
        delays = [delay]
        for additional_delay in multipath_delays:
            delays.append(delay + additional_delay)

        # Create powers: primary path + multipath components
        powers = [1.0]  # Primary path has power 1.0
        for tap in multipath_taps:
            powers.append(tap**2)  # Convert amplitude to power
