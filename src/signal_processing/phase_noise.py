...
from ..core.types import Hertz, Decibels, Picoseconds
...
class PhaseNoiseModel:
    ...
    def get_phase_noise_at(self, offset_freq: Hertz) -> Decibels:
        ...
        if offset_freq < self.flicker_corner_freq:
            flicker_noise = self.flicker_noise_level + 20 * np.log10(self.flicker_corner_freq / offset_freq)
        else:
            flicker_noise = self.flicker_noise_level
        
        # Random walk (1/f²)
        if offset_freq < self.random_walk_corner_freq:
            random_walk_noise = self.random_walk_level + 40 * np.log10(self.random_walk_corner_freq / offset_freq)
        else:
    ...
    def get_phase_noise_profile(self, 
                               freq_points: Optional[List[Hertz]] = None) -> Dict[Hertz, Decibels]:
        """
    ...
    def generate_phase_noise_time_series(self, 
                                       duration: float, 
                                       sampling_rate: Hertz) -> np.ndarray:
        """
        ...
        shaped_noise_freq = noise_freq * np.sqrt(phase_noise_spectrum * sampling_rate / 2)
    ...
    def create_tcxo_model(cls, 
                          ...
                          random_walk: Decibels = -100.0) -> 'PhaseNoiseModel':
        """
    ...
    def create_ocxo_model(cls, 
                          ...
                          random_walk: Decibels = -110.0) -> 'PhaseNoiseModel':
        """
    ...
    @classmethod
    def create_custom_model(cls,
                           noise_params: Dict[str, float]) -> 'PhaseNoiseModel':
        """
...
class PhaseNoiseSweep:
    ...
    @staticmethod
    def generate_sweep_parameters(param_name: str, 
                                start_value: float, 
                                end_value: float, 
    ...
    @staticmethod
    def create_phase_noise_models_from_sweep(param_name: str, 
                                           values: List[float],
                                           base_model: PhaseNoiseModel) -> List[PhaseNoiseModel]:
        """
