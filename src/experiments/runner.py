...
from ..core.types import (PPB, ExperimentConfig, ExperimentResult,
                          MeasurementQuality, PerformanceMetrics, Picoseconds,
                          Seconds, Timestamp)
...
class ExperimentRunner:
    ...
    def run_single_experiment(
        self, experiment_func, parameters: Dict[str, Any]
    ) -> ExperimentResult:
        """
    ...
    def run_parameter_sweep(
        self, experiment_func, parameter_ranges: Dict[str, List[Any]]
    ...
    def run_monte_carlo(
        self, experiment_func, n_runs: int, parameter_distributions: Dict[str, Any]
    ) -> List[ExperimentResult]:
        """
        ...
        if self.context.verbose:
            print(
                f"Starting Monte Carlo campaign for {self.context.config.experiment_id}"
            )
        ...
        for i in range(n_runs):
            ...
            updated_context = ExperimentContext(
                ...
                output_dir=self.context.output_dir,
                random_seed=(
                    self.context.random_seed + i if self.context.random_seed else None
                ),
                verbose=self.context.verbose,
            )
    ...
    def get_summary_statistics(
        self, results: Optional[List[ExperimentResult]] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Extract metrics
        timing_errors = [
            r.metrics.rmse_timing
            for r in successful_results
            if r.metrics.rmse_timing != float("inf")
        ]
        frequency_errors = [
            r.metrics.rmse_frequency
            for r in successful_results
            if r.metrics.rmse_frequency != float("inf")
        ]
        convergence_times = [
            r.metrics.convergence_time
            for r in successful_results
            if r.metrics.convergence_time != float("inf")
        ]
