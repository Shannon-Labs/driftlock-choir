...
from ..core.types import (
    BeatNoteData, EstimationResult, Hertz, Picoseconds, 
    Timestamp, MeasurementQuality, PhaseMeasurement
)
...
class PhaseSlopeEstimator(TauDeltaEstimator):
    ...
    def compute_uncertainty(self, measurements: List[PhaseMeasurement]) -> np.ndarray:
        ...
        W = np.diag(weights)
        ATA = A.T @ W @ A
        covariance = np.linalg.inv(ATA) if np.linalg.cond(ATA) < 1e12 else np.eye(2) * 1e6
...
class MaximumLikelihoodEstimator(TauDeltaEstimator):
    ...
    def estimate(self, beat_note: BeatNoteData) -> EstimationResult:
        ...
        def negative_likelihood(params):
            ...
            expected_signal = self._generate_expected_signal(beat_note, tau_ps, delta_f_hz)
    ...
    def _generate_expected_signal(self, beat_note: BeatNoteData, tau_ps: float, delta_f_hz: float) -> np.ndarray:
        """
        ...
        expected_signal = np.exp(1j * (2 * np.pi * beat_freq * t + 2 * np.pi * beat_freq * tau_seconds))
...
class CRLBEstimator(TauDeltaEstimator):
    ...
    def _compute_crlb(self, beat_note: BeatNoteData) -> Tuple[float, float]:
        ...
        crlb_delta_f = 3.0 / (2 * np.pi**2 * snr_linear * n_samples * (n_samples**2 - 1))
        
        # CRLB for time delay estimation
        if beat_freq > 0:
            crlb_tau = 1.0 / (8 * np.pi**2 * snr_linear * beat_freq**2 * n_samples) * PhysicalConstants.PS_PER_SEC
        else:
            crlb_tau = float('inf')
