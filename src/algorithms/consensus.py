...
from ..core.types import (PPB, ConsensusState, EstimationResult, Hertz,
                          MeasurementQuality, NetworkTopology, NodeState,
                          Picoseconds, Seconds, Timestamp)
...
class ConsensusMessage:
    ...
    def get_weight(self, method: str = "inverse_variance") -> float:
        ...
        if method == "inverse_variance":
            ...
            delta_f_weight = (
                1.0 / (delta_f_var / nominal_delta_f**2) if delta_f_var > 0 else 0.0
            )
...
class ConsensusAlgorithm(ABC):
    ...
    @abstractmethod
    def initialize(
        self, node_states: List[NodeState], topology: NetworkTopology
    ) -> ConsensusState:
        """
    ...
    @abstractmethod
    def step(
        self, consensus_state: ConsensusState, messages: List[ConsensusMessage]
    ) -> ConsensusState:
        """
...
class MetropolisConsensus(ConsensusAlgorithm):
    ...
    def __init__(self, convergence_threshold: float = 1e-3, max_iterations: int = 100):
        """
    ...
    def step(
        ...
        for node_state in consensus_state.node_states:
            ...
            new_tau_uncertainty = (
                node_state.clock_bias_uncertainty * 0.9
            )  # Assume some improvement
...
class InverseVarianceConsensus(ConsensusAlgorithm):
    ...
    def step(
        ...
        for node_state in consensus_state.node_states:
            ...
            delta_f_sum = node_state.frequency_offset / (
                node_state.frequency_offset_uncertainty**2
            )
            ...
            for message in node_messages:
                ...
                if message.delta_f_uncertainty > 0:
                    delta_f_sum += message.delta_f_estimate / (
                        message.delta_f_uncertainty**2
                    )
                    weight_sum_delta_f += 1.0 / (message.delta_f_uncertainty**2)

            # Normalize
            new_tau = (
                tau_sum / weight_sum_tau
                if weight_sum_tau > 0
                else node_state.clock_bias
            )
            new_delta_f = (
                delta_f_sum / weight_sum_delta_f
                if weight_sum_delta_f > 0
                else node_state.frequency_offset
            )

            # Update uncertainty (inverse of sum of inverse variances)
            new_tau_uncertainty = (
                np.sqrt(1.0 / weight_sum_tau)
                if weight_sum_tau > 0
                else node_state.clock_bias_uncertainty
            )
            new_delta_f_uncertainty = (
                np.sqrt(1.0 / weight_sum_delta_f)
                if weight_sum_delta_f > 0
                else node_state.frequency_offset_uncertainty
            )
    ...
    def _calculate_convergence_metric(self, node_states: List[NodeState]) -> float:
        ...
        timing_weights = [
            (
                1.0 / (float(state.clock_bias_uncertainty) ** 2)
                if state.clock_bias_uncertainty > 0
                else 1.0
            )
        ...
        frequency_weights = [
            (
                1.0 / (float(state.frequency_offset_uncertainty) ** 2)
                if state.frequency_offset_uncertainty > 0
                else 1.0
            )
            for state in node_states
        ]
        ...
        timing_var = np.average(
            [(x - timing_mean) ** 2 for x in timing_errors], weights=timing_weights
        )
        frequency_var = np.average(
            [(x - frequency_mean) ** 2 for x in frequency_errors],
...
class ConsensusSimulator:
        ...
        for i, sender_state in enumerate(consensus_state.node_states):
            ...
            for neighbor_id in neighbors:
                ...
                message = ConsensusMessage(
                    ...
                    delta_f_estimate=Hertz(
                        sender_state.frequency_offset
                        * sender_state.get_frequency_hz(2.4e9)
                        * 1e-9
                    ),
                    delta_f_uncertainty=Hertz(
                        sender_state.frequency_offset_uncertainty
                        * sender_state.get_frequency_hz(2.4e9)
                        * 1e-9
                    ),
    ...
    def get_convergence_metrics(self) -> Dict[str, List[float]]:
        ...
        for consensus_state in self.consensus_history:
            ...
            metrics["max_timing_error"].append(
                float(consensus_state.get_max_timing_error())
            )
            metrics["max_frequency_error"].append(
                float(consensus_state.get_max_frequency_error())
            )

            # Calculate average errors
            timing_errors = [
                float(state.clock_bias) for state in consensus_state.node_states
            ]
            frequency_errors = [
                float(state.frequency_offset) for state in consensus_state.node_states
            ]
