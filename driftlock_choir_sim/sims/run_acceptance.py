...
from driftlock_choir_sim.dsp.impairments import apply_cfo, apply_phase_noise, apply_sco
from driftlock_choir_sim.dsp.rx_coherent import (estimate_tone_phasors, unwrap_phase, wls_delay,
                                          per_tone_snr, estimate_noise_power)
from driftlock_choir_sim.dsp.rx_aperture import envelope_spectrum, detect_df_peak
from driftlock_choir_sim.dsp.crlb import delay_crlb_std, delay_crlb_rms_bandwidth
...
OUT = Path("driftlock_choir_sim/outputs")
...
def acceptance_aperture(seed: int = 2025) -> dict:
...
def acceptance_coherent_precision(seed: int = 2026) -> dict:
    ...
    from driftlock_choir_sim.dsp.phase_schedules import amplitude_taper
...
def acceptance_robustness(seed: int = 2027) -> dict:
    rng = np.random.default_rng(seed)
    fs = 5e6
    dur = 0.02  # Reduced duration for speed
    df = 10e3
...
def acceptance_payload(seed: int = 2028) -> dict:
    fs = 5e6
    ...
    def run(payload_frac: float, subseed: int) -> Tuple[float, float, float]:
        tau_errs: list[float] = []
        total_bits = 0
        total_bit_errs = 0

        telemetry_path = OUT / "csv" / f"payload_telemetry_{payload_frac}.jsonl"
        with TelemetryCollector(telemetry_path) as collector:
            for i in range(trials):
                r = np.random.default_rng(subseed + i)
                ...
                # Add to telemetry (simplified for payload test)
                rmse_ps = float(np.sqrt(np.mean((np.asarray(tau_errs) - 0.0) ** 2)))
                collector.add_frame(i, tau_hat * 1e12, 0.0, 0.0, 0.0, rmse_ps, 0.0)
    ...
    rmse_no, _, _ = run(0.0, seed)
    rmse_pl, obs_ber, ana_ber = run(1.0 / 8.0, seed + 1000)
    ...
    return {
        ...
        "telemetry_path": str(OUT / "csv" / "payload_telemetry_0.125.jsonl"),
...
def main() -> None:
    ...
    # JSONL for streaming per test
    jsonl_path = OUT / "csv" / "acceptance_tests.jsonl"
    ensure_dir(OUT / "csv")

    # CSV summary
    csv_path = OUT / "csv" / "acceptance_summary.csv"
    fieldnames = [
        'test_type', 'rng_seed', 'env_df_snr_dB', 'env_2df_snr_dB', 'df_hz', 'passed',
        'rmse_ps', 'crlb_ps', 'ratio', 'ci_coverage', 'unwrap_sanity_rate', 'beff_rms_hz',
        'fs_hz', 'duration_s', 'n_carriers', 'truth_tau_ps', 'n_trials',
        'tau_hat_ps', 'rmse_no_payload_ps', 'rmse_with_payload_ps', 'worsening_pct',
        'observed_ber', 'analytic_ber'
    ]
    ...
    def append_to_jsonl(result: dict):
