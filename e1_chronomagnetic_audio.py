...
def main():
    ...
    sf.write(os.path.join(output_dir, "e1_beat_note_formation.wav"), e1_beat_note, sr)
    ...
    sf.write(os.path.join(output_dir, "e1_chronomagnetic_pulses.wav"), pulse_signal, sr)

    # 3. τ and Δf modulation
    print("3. Generating τ/Δf modulation audio...")
    tau_delta_f_signal, sr = generate_tau_delta_f_modulation()
    sf.write(
        os.path.join(output_dir, "e1_tau_delta_f_modulation.wav"),
        tau_delta_f_signal,
        sr,
    )
    ...
    print(
        "- e1_chronomagnetic_pulses.wav: Information 'materializing' at E1-relevant frequencies"
    )
    print(
        "- e1_tau_delta_f_modulation.wav: Modulation representing τ/Δf relationship from E1"
    )
    print(
    ...
    print(
        "at specific temporal frequencies representing timing (τ) and frequency (Δf) measurements."
    )
    print(
        f"\\nE1 achieved: {2.1} ps timing precision and {0.8} ppb frequency precision."
    )
    print(
        "Listen for the beat patterns and modulations that represent these measurements."
    )
