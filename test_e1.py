...
def test_signal_generation():
    ...
    try:
        ...
        if len(signal) == expected_samples:
            ...
            print(f"✗ Signal length mismatch: expected {expected_samples}, got {len(signal)}")
            return False
...
def test_beat_note_processing():
    ...
    try:
        ...
        _, rx_signal = rx_oscillator.generate_signal(duration, sampling_rate, Hertz(50.0))
        ...
        if abs(beat_freq - expected_beat_freq) < 10.0:
            ...
            print(f"✗ Beat frequency mismatch: expected ~{expected_beat_freq}, got {beat_freq}")
            return False
        
        # Estimate τ and Δf
        estimation_result = processor.estimate_tau_delta_f(beat_note)
        print(f"✓ τ estimated: {estimation_result.tau:.1f} ± {estimation_result.tau_uncertainty:.1f} ps")
        print(f"✓ Δf estimated: {estimation_result.delta_f:.1f} ± {estimation_result.delta_f_uncertainty:.1f} Hz")
...
def test_algorithms():
    ...
    try:
        ...
        for estimator_type in available_estimators:
            ...
            print(f"✓ {estimator_type} estimator: τ={result.tau:.1f}ps, Δf={result.delta_f:.1f}Hz")
