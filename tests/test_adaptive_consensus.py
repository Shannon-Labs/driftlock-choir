...
try:
    from src.algorithms.adaptive_consensus import (AdaptiveConsensus,
                                                   AdaptiveConsensusState,
                                                   AdaptiveTrackingResult)
except ModuleNotFoundError:  # pragma: no cover - optional feature
    pytest.skip(
        "Adaptive consensus module is not available in the open-source release.",
        allow_module_level=True,
    )

from src.algorithms.consensus import ConsensusMessage
from src.core.constants import PhysicalConstants
from src.core.types import (PPB, ConsensusState, Hertz, MeasurementQuality,
                            NetworkTopology, NodeState, Picoseconds, Timestamp)
...
class TestAdaptiveConsensus(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.adaptive_consensus = AdaptiveConsensus(
            convergence_threshold=1e-3, max_iterations=100, adaptation_enabled=True
        )
    ...
    def test_initialization(self):
        ...
        consensus = AdaptiveConsensus(
            convergence_threshold=1e-4, max_iterations=200, adaptation_enabled=True
        )
    ...
    def test_initialize(self):
        ...
        initial_state = self.adaptive_consensus.initialize(
            self.node_states, self.topology
        )
        ...
        self.assertIsInstance(
            self.adaptive_consensus.adaptive_state, AdaptiveConsensusState
        )
        self.assertEqual(
            self.adaptive_consensus.adaptive_state.adaptation_enabled, True
        )
    ...
    def test_step_with_adaptation_enabled(self):
        ...
        self.assertIsNotNone(self.adaptive_consensus.adaptive_state)
        self.assertEqual(
            self.adaptive_consensus.adaptive_state.consensus_state, new_state
        )
        self.assertGreater(len(self.adaptive_consensus.consensus_history), 1)

        # Check tracking history
        self.assertGreater(
            len(self.adaptive_consensus.adaptive_state.tracking_history), 0
        )
    ...
    def test_variance_safeguards(self):
        ...
        safeguarded_state = self.adaptive_consensus._apply_variance_safeguards(
            high_variance_state
        )
        ...
        if self.adaptive_consensus.adaptive_state.variance_result.warning_level in [
            "warning",
            "critical",
        ]:
            self.assertTrue(
                self.adaptive_consensus.adaptive_state.reweighting_result is not None
                or self.adaptive_consensus.adaptive_state.saturation_result is not None
            )
    ...
    def test_ml_predictions(self):
        ...
        with patch.object(
            self.adaptive_consensus.ml_hooks, "predict_weights"
        ) as mock_weights, patch.object(
            self.adaptive_consensus.ml_hooks, "predict_step_size"
        ) as mock_step_size, patch.object(
            self.adaptive_consensus.ml_hooks, "predict_stability"
        ) as mock_stability:
            ...
            self.assertTrue(
                self.adaptive_consensus.adaptive_state.ml_predictions_used.get(
                    "weights", False
                )
            )
            self.assertTrue(
    ...
    def test_fallback_mechanism(self):
        ...
        with patch.object(
            self.adaptive_consensus, "_perform_adaptive_step"
        ) as mock_step:
    ...
    def test_convergence_with_various_conditions(self):
        ...
        for i in range(3):
            ...
            # Create consensus
            consensus = AdaptiveConsensus(
                convergence_threshold=1e-3, max_iterations=50, adaptation_enabled=True
            )
            ...
            for _ in range(50):
                ...
                for sender_state in current_state.node_states:
                    neighbors = current_state.topology.get_neighbors(
                        sender_state.node_id
                    )
            ...
            self.assertTrue(
                consensus.has_converged(current_state) or current_state.iteration >= 50
            )
