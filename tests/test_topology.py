...
import numpy as np
import warnings
from src.simulation.topology import TopologyGenerator, TopologyAnalysis
...
class TestTopologyGenerator(unittest.TestCase):
    ...
    def test_generate_random_geometric_topology_valid(self):
        ...
        self.assertTrue(np.allclose(topology.adjacency_matrix, topology.adjacency_matrix.T))
    ...
    def test_generate_random_geometric_topology_disconnected(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            topology = self.generator.generate_random_geometric_topology(20, 0.1, seed=42)
    ...
    def test_estimate_consensus_difficulty_connected(self):
        ...
        topology = self.generator.generate_ring_topology(5)
        difficulty = self.generator._estimate_consensus_difficulty(topology)
        self.assertIn(difficulty, ["Easy (well-connected)", "Moderate (reasonably connected)"])
        
        # Poorly connected topology (line)
        topology = self.generator.generate_line_topology(10)
        difficulty = self.generator._estimate_consensus_difficulty(topology)
        self.assertIn(difficulty, ["Moderate (reasonably connected)", "Challenging (poorly connected)"])
