...
def plot_oscillator_comparison(demos):
    ...
    try:
        ...
        axes[0, 0].plot(
            time_ms, ideal_signal[:plot_samples].real, "b-", label="Ideal", linewidth=1
        )
        axes[0, 0].plot(
            time_ms, tcxo_signal[:plot_samples].real, "r-", label="TCXO", linewidth=1
        )
        ...
        axes[0, 1].plot(
            time_ms,
            np.abs(ideal_signal[:plot_samples]),
            "b-",
            label="Ideal",
            linewidth=1,
        )
        axes[0, 1].plot(
            time_ms, np.abs(tcxo_signal[:plot_samples]), "r-", label="TCXO", linewidth=1
        )
        ...
        axes[1, 0].plot(
            time_ms,
            np.angle(ideal_signal[:plot_samples]),
            "b-",
            label="Ideal",
            linewidth=1,
        )
        axes[1, 0].plot(
            time_ms,
            np.angle(tcxo_signal[:plot_samples]),
            "r-",
            label="TCXO",
            linewidth=1,
        )
...
def plot_channel_effects(time, clean_signal, noisy_signals):
    ...
    try:
        ...
        axes[0, 0].plot(
            time_us, clean_signal[:plot_samples].real, "b-", label="Clean", linewidth=2
        )
        axes[0, 0].plot(
            time_us,
            noisy_signals[20][:plot_samples].real,
            "r-",
            alpha=0.7,
            label="SNR 20dB",
        )
        ...
        axes[0, 1].plot(
            time_us,
            np.abs(clean_signal[:plot_samples]),
            "b-",
            label="Clean",
            linewidth=2,
        )
        axes[0, 1].plot(
            time_us,
            np.abs(noisy_signals[20][:plot_samples]),
            "r-",
            alpha=0.7,
            label="SNR 20dB",
        )
        ...
        axes[1, 0].scatter(
            clean_signal[:plot_samples].real,
            clean_signal[:plot_samples].imag,
            c="b",
            alpha=0.5,
            s=1,
            label="Clean",
        ...
        axes[1, 0].scatter(
            noisy_signals[20][:plot_samples].real,
            noisy_signals[20][:plot_samples].imag,
            c="r",
            alpha=0.3,
            s=1,
            label="SNR 20dB",
        ...
        for snr in snr_values:
            ...
            correlation = np.abs(
                np.corrcoef(clean_signal.real, noisy_signals[snr].real)[0, 1]
            )
