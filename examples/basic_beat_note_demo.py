...
def main():
    ...
    print(f"Estimated τ: {estimation_result.tau:.1f} ± {estimation_result.tau_uncertainty:.1f} ps")
    print(f"Estimated Δf: {estimation_result.delta_f:.1f} ± {estimation_result.delta_f_uncertainty:.1f} Hz")
    ...
    # Generate plots
    print("\nGenerating plots...")
    plot_beat_note_analysis(beat_note, beat_processor, estimation_result, true_tau, true_delta_f)
    
    print("\nDemo completed!")


def plot_beat_note_analysis(beat_note, processor, estimation_result, true_tau, true_delta_f):
    """Generate comprehensive plots of beat note analysis."""
    try:
        # Create figure with subplots
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle(f"Beat Note Analysis Demo\n"
        ...
        axes[0, 1].plot(plot_freqs, 20 * np.log10(plot_magnitude + 1e-10), 'r-', linewidth=1)
        axes[0, 1].axvline(x=beat_note.get_beat_frequency(), color='g', linestyle='--', 
