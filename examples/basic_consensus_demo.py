...
from src.algorithms.consensus import ConsensusSimulator, MetropolisConsensus
from src.core.types import (PPB, NetworkTopology, NodeState, Picoseconds,
                            Timestamp)
...
def run_consensus_demo():
    ...
    consensus_algorithm = MetropolisConsensus(
        convergence_threshold=1e-3, max_iterations=20  # 1 ps convergence threshold
    )
    ...
    consensus_history = simulator.run_simulation(
        initial_states=initial_states, topology=topology, max_iterations=20
    )
    ...
    for i, state in enumerate(consensus_history):
        ...
        print(
            f"{i:4d} | {node0_bias:10.1f}ps | {node1_bias:10.1f}ps | {convergence_metric:.6f}"
        )
